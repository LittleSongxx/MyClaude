...
from myclaude.conversation import ConversationManager, Message, ToolResultBlock
...
def test_replacement_idempotent(tmp_path: Path) -> None:
    ...
    content_after_first = conv.history[0].tool_results[0].content

    recs2 = apply_tool_result_budget(conv, tmp_path, state)
    content_after_second = conv.history[0].tool_results[0].content

    assert content_after_first == content_after_second, "second pass must be idempotent"
    assert len(recs1) == 1
    assert recs1[0].replacement == content_after_first
    # 第二次只是纯粹的重新应用：不产生新记录
    assert recs2 == []
...
def test_frozen_never_replaced(tmp_path: Path) -> None:
    ...
    t1 = next(tr for tr in conv.history[0].tool_results if tr.tool_use_id == "t1")
