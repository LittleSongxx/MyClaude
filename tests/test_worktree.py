...
from myclaude.config import WorktreeConfig, load_config
from myclaude.worktree.changes import count_worktree_changes, has_worktree_changes
from myclaude.worktree.integration import build_worktree_notice, generate_worktree_name
from myclaude.worktree.manager import WorktreeError, WorktreeManager
from myclaude.worktree.models import WorktreeSession
...
class TestIntegrationHelpers:
    def test_generate_worktree_name(self):
        ...
        # "agent-a"(7字符) + token_hex(4)[:7](7位十六进制) = 14 字符
...
class TestWorktreeManager:
    ...
    @pytest.mark.asyncio
    async def test_enter_and_session(self, manager):
    ...
    @pytest.mark.asyncio
    async def test_enter_sets_session(self, manager):
        await manager.create("session-test")
        await manager.enter("session-test")
        assert manager.current_session is not None
        assert manager.current_session.worktree_name == "session-test"

    @pytest.mark.asyncio
    async def test_exit_keep(self, manager):
