...
class TestSafeCommandClassification:
    def test_minimal_read_only_commands(self) -> None:
    ...
    def test_commands_that_can_escape_or_exfiltrate_require_approval(
        self, command: str
    ) -> None:
...
class TestPermissionChecker:
    ...
    def test_plan_mode_allows_only_exact_plan_file(self) -> None:
        ...
        allowed = self.checker.check(
            tool, {"file_path": str(plan_path), "content": "plan"}
        )
        sibling = self.checker.check(
            tool,
            {"file_path": str(plan_path.parent / "other.md"), "content": "x"},
        )
    ...
    def test_explicit_deny_overrides_safe_command_allowlist(self) -> None:
        ...
        rules_file.write_text(
            yaml.dump([{"rule": "Bash(git status*)", "effect": "deny"}])
        )
        checker = PermissionChecker(
            detector=DangerousCommandDetector(),
            sandbox=PathSandbox(str(self.tmpdir)),
...
async def test_e2e_rule_allows_git():
    ...
    client = MockLLMClient([
        [
            ToolCallComplete("t1", "Bash", {"command": "git --version"}),
            StreamEnd("end_turn", input_tokens=10, output_tokens=20),
        ],
        [
            TextDelta("Done."),
    ...
    agent = Agent(client, registry, "anthropic", work_dir=str(tmpdir), permission_checker=checker)
    conv = ConversationManager()
    conv.add_user_message("Show the git version")
