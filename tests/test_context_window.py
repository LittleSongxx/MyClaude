...
from myclaude.validator import (
    ConfigError,
...
def test_native_tool_search_model_compatibility(model: str, expected: bool) -> None:
    assert supports_anthropic_tool_search(model) is expected
...
class TestAutoFetch:
    ...
    async def test_fetch_success_is_cached_and_used(self):
        ...
        with patch("myclaude.client.create_client", return_value=fake) as mk:
    ...
    async def test_fetch_raises_degrades_to_mapping_table(self):
        ...
        with patch("myclaude.client.create_client", return_value=fake):
            # 不应抛出异常。
            await resolve_context_window(p)
        # 对 claude 回退到映射表。
    ...
    async def test_non_anthropic_provider_is_not_fetched(self):
        ...
        with patch("myclaude.client.create_client") as mk:
            await resolve_context_window(p)
