"""从 .myclaude/commands/ 目录加载自定义 Markdown 命令（对齐 Go 版 loader.go）。
...
def load_user_commands(work_dir: str, *, include_project: bool = True) -> list[Command]:
    """从用户全局和项目级目录合并加载自定义命令（对齐 Go 版 LoadUserCommands）。

    搜索路径（后者覆盖前者）：
      1. ~/.myclaude/commands/
      2. <work_dir>/.myclaude/commands/
    """
    dirs: list[str] = []
    home = Path.home()
    dirs.append(str(home / ".myclaude" / "commands"))
    if include_project:
        dirs.append(str(Path(work_dir) / ".myclaude" / "commands"))
...
def register_user_commands(
    ...
    *,
    ...
    """Register trusted custom commands without overriding built-in commands.

    Returns human-readable conflict messages so delivery surfaces can log or
    display them without making one bad command prevent startup.
    """

    errors: list[str] = []
    for command in load_user_commands(work_dir, include_project=include_project):
        try:
