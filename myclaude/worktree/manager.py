...
from myclaude.worktree.setup import perform_post_creation_setup
...
class WorktreeManager:
    def __init__(
        ...
        self.worktree_dir = worktree_dir or str(
            Path(repo_root) / ".myclaude" / "worktrees"
        )
        self._myclaude_dir = Path(repo_root) / ".myclaude"
    ...
    async def enter(self, name: str) -> WorktreeSession:
        ...
        original_cwd = str(Path(self.repo_root).expanduser().resolve())
        ...
        save_worktree_session(self._myclaude_dir, session)
        return session
    ...
    async def exit(
        ...
        save_worktree_session(self._myclaude_dir, None)
    ...
    def restore_session(self) -> WorktreeSession | None:
        session = load_worktree_session(self._myclaude_dir)
        if session is None:
            return None
