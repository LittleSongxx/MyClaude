...
def _session_path(myclaude_dir: Path) -> Path:
    return myclaude_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(myclaude_dir)
    if session is None:
        # 传入 None 时直接删除文件而非写空 JSON，
...
def load_worktree_session(myclaude_dir: Path) -> WorktreeSession | None:
    path = _session_path(myclaude_dir)
    if not path.exists():
        return None
    try:
