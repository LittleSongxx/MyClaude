...
from myclaude.commands.completion import CompletionPopup
...
from myclaude.agents.notification import inject_task_notifications
from myclaude.commands.handlers.tasks import create_tasks_command
from myclaude.skills.executor import SkillExecutor
from myclaude.skills.loader import SkillLoader
from myclaude.commands.handlers.skill_register import register_skill_commands
...
if TYPE_CHECKING:
    ...
    from myclaude.plan_dialog import InlinePlanWidget
from myclaude.worktree.cleanup import start_stale_cleanup_task
from myclaude.worktree.manager import WorktreeManager
from myclaude.commands.handlers.worktree import create_worktree_command
from myclaude.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".myclaude", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".myclaude" / "history"
...
def _tool_title(tool_name: str, arguments: dict[str, Any]) -> str:
    ...
    if tool_name == "DeleteFile":
        path = os.path.basename(arguments.get("file_path", ""))
        return f"Delete {path}" if path else "Delete"
    if tool_name == "Bash":
...
class MyClaudeApp(App):
    ...
    def __init__(
        ...
        from myclaude.config import SandboxAppConfig
        ...
        self._startup_hook_task: asyncio.Task[Any] | None = None
    ...
    def _make_banner(model: str = "", work_dir: str = "") -> RichText:
        ...
        t.append(f"MyClaude v{__version__}\n", style="color(242)")
    ...
    def on_mount(self) -> None:
        self.register_theme(_MYCLAUDE_THEME)
        self.theme = "myclaude"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        self._load_skill_tool = features.load_skill_tool
        ...
        # --- Worktree 系统初始化 ---
        from myclaude.config import WorktreeConfig
    ...
    async def on_chat_input_submitted(self, event: ChatInput.Submitted) -> None:
        ...
        if self._streaming and not text.startswith("/"):
            if self.agent is not None and text:
                position = self.agent.queue_user_message(text)
                self.query_one("#chat-input", ChatInput).clear()
                self._show_system_message(f"Message queued ({position})")
            return
    ...
    async def _prefetch_relevant_memories(self, query: str) -> str:
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            ...
            ledger = self.client.usage_ledger if self.client is not None else None
    ...
    async def _send_message(self, text: str, is_notification: bool = False) -> None:
        ...
        if prefetch_task is not None:
            previous_recall = self.agent.memory_recall_task
            if previous_recall is not None and not previous_recall.done():
                previous_recall.cancel()
    ...
    def on_inline_plan_widget_responded(
        ...
        from myclaude.plan_dialog import InlinePlanWidget, PlanChoice
    ...
    def _schedule_session_summary(self) -> None:
        """Debounce summary generation and isolate it from mutable UI state."""
        if not self.session or not self.client or not self.agent:
            return
        if self._session_summary_task and not self._session_summary_task.done():
            self._session_summary_task.cancel()
    ...
    async def _init_mcp(self) -> None:
        ...
        mcp_tools = len(connect_result.tools)
