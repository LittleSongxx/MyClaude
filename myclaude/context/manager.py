...
SESSION_SUBDIR = ".myclaude/session/tool-results"
...
def ensure_session_dir(work_dir: str, scope_id: str = "default") -> Path:
    """Return an isolated spill directory for one agent run.

    Tool-result replacement records must not be shared by concurrent agents.  A
    conservative scope normalisation also prevents a caller-controlled id from
    escaping the project-local state directory.
    """
    safe_scope = "".join(
        char for char in scope_id if char.isascii() and (char.isalnum() or char in "-_")
    ) or "default"
    session_dir = Path(work_dir) / SESSION_SUBDIR / safe_scope
...
def cleanup_tool_results(
    ...
    """Remove spill files that are no longer referenced by live messages."""
    ...
    for entry in session_dir.iterdir():
        if entry.name == REPLACEMENT_RECORDS_FILENAME:
            continue
        try:
...
def referenced_persisted_paths(
    messages: list[Message], session_dir: Path
) -> set[Path]:
    """Extract spill paths from persisted-output previews in retained messages."""
    ...
    for message in messages:
        ...
        texts.extend(result.content for result in message.tool_results)
    for text in texts:
        if PERSISTED_TAG not in text:
            continue
...
def compute_compact_threshold(context_window: int, manual: bool = False) -> int:
    if context_window <= 0:
        return 0
    margin = (
        MANUAL_COMPACT_SAFETY_MARGIN if manual else AUTO_COMPACT_SAFETY_MARGIN
    )
    configured = context_window - SUMMARY_OUTPUT_RESERVE - margin
    # Preserve the intended output/safety reserve on normal model windows, but
    # never produce a negative trigger for small/local models.
    return max(1, context_window // 2, configured)
...
async def _request_summary(client: Any, messages: list[Message]) -> str:
    from myclaude.tools.base import StreamEnd, TextDelta

    summary_conv = ConversationManager()
    summary_conv.history = list(messages) + [
        Message(
            role="user",
            content=(
                "Provide the summary now. Return plain text only, using the "
                "required <analysis> and <summary> blocks."
            ),
        )
    ]
    collected = ""
    completed = False
    # Summary requests never receive tools.  Prompt-only prohibitions are not a
    # security boundary and previously allowed accidental tool calls to replace
    # history with an empty summary.
    scope = getattr(client, "usage_scope", None)
    with scope("compact") if scope is not None else nullcontext():
        async for event in client.stream(
            summary_conv, system=SUMMARY_PROMPT, tools=None
        ):
            if isinstance(event, TextDelta):
                collected += event.text
            elif isinstance(event, StreamEnd):
                completed = event.stop_reason in ("", "end_turn", "stop")
    if not completed:
        raise ValueError("summary model did not finish normally")
...
async def _summarize_messages(
    client: Any, messages: list[Message], depth: int = 0
) -> str:
    try:
        return await _request_summary(client, messages)
    except Exception as error:
        groups = _group_messages_by_turn(messages)
        if (
            not _is_context_length_error(error)
            or len(groups) < 2
            or depth >= 4
        ):
            raise
        ...
        left_messages = [message for group in groups[:midpoint] for message in group]
        right_messages = [message for group in groups[midpoint:] for message in group]
        left = extract_summary(
            await _summarize_messages(client, left_messages, depth + 1)
        )
        right = extract_summary(
            await _summarize_messages(client, right_messages, depth + 1)
        )
        combined = [
            Message(role="user", content=f"Earlier segment summary:\n{left}"),
            Message(role="user", content=f"Later segment summary:\n{right}"),
        ]
        return await _request_summary(client, combined)
...
async def auto_compact(
    ...
    try:
        llm_output = await _summarize_messages(client, list(to_summarize))
    except Exception as error:
    ...
    cleanup_tool_results(
        session_dir,
        preserve_paths=referenced_persisted_paths(list(keep_tail), session_dir),
    )
