...
from myclaude.hooks.models import Action, ActionResult, HookContext
...
async def execute_command(action: Action, ctx: HookContext) -> ActionResult:
    ...
    try:
        ...
        env = os.environ.copy()
        env["MYCLAUDE_HOOK_CONTEXT"] = payload
        ...
        try:
            stdout, _ = await asyncio.wait_for(
                proc.communicate(payload.encode("utf-8")), timeout=action.timeout
            )
        ...
        if isinstance(structured, dict):
            raw_decision = structured.get("decision", "none")
            if raw_decision in {"allow", "deny"}:
