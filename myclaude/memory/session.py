    ...
    def from_jsonl(cls, line: str) -> SessionRecord | None:
        try:
            ...
            if not isinstance(data, dict):
                return None
            data = _migrate_record_data(data)
            return cls(
...
def _migrate_record_data(data: dict[str, Any]) -> dict[str, Any]:
    version = data.get("schema_version", 0)
    if not isinstance(version, int) or isinstance(version, bool) or version < 0:
        raise ValueError("invalid session record schema version")
    if version > SESSION_SCHEMA_VERSION:
        raise ValueError("session record uses a newer schema")
    migrated = dict(data)
    if version == 0:
        # Pre-versioned transcripts already used the current field layout.
...
async def generate_session_summary(
    ...
    try:
        ...
        with scope("session-summary") if scope is not None else nullcontext():
            async for event in client.stream(
                summary_conv, system=SESSION_SUMMARY_PROMPT, tools=None
            ):
    ...
    return collected.strip() if completed else ""
...
class SessionManager:
    ...
    def resume(self, session_id: str) -> ResumeResult | None:
        ...
        meta = SessionMeta.load(meta_path)
        if meta is None:
            # A torn or missing metadata file must not make an otherwise valid
            # append-only transcript unrecoverable.
