...
def get_auto_mem_path(project_root: str) -> str:
    """返回项目级记忆目录路径：<projectRoot>/.myclaude/memory/。

    保留尾部分隔符，确保前缀匹配不会误命中类似 memoryxyz 的路径。
    支持 MYCLAUDE_REMOTE_MEMORY_DIR 环境变量覆盖。
    """
    override = os.environ.get("MYCLAUDE_REMOTE_MEMORY_DIR", "")
    if override:
        return override.rstrip(os.sep) + os.sep
    abs_root = os.path.abspath(project_root)
    return os.path.join(abs_root, ".myclaude", "memory") + os.sep


def get_user_auto_mem_path() -> str:
    """返回用户级记忆目录路径：~/.myclaude/memory/。
    ...
    if not home:
        return ""
    return os.path.join(home, ".myclaude", "memory") + os.sep
...
def is_auto_mem_path(absolute_path: str, project_root: str) -> bool:
    ...
    try:
        ...
        roots = [get_auto_mem_path(project_root), get_user_auto_mem_path()]
        return any(
            root and candidate.is_relative_to(Path(root).expanduser().resolve())
            for root in roots
        )
    except OSError:
        return False
...
def truncate_entrypoint_content(raw: str) -> str:
    ...
    if len(result_bytes) > MAX_ENTRYPOINT_BYTES:
        byte_limited = result_bytes[:MAX_ENTRYPOINT_BYTES].decode(
            "utf-8", errors="ignore"
        )
        cut = byte_limited.rfind("\n")
        if cut > 0:
            result = byte_limited[:cut]
        else:
            result = byte_limited
...
class MemoryManager:
    ...
    def user_path(self) -> Path:
        ...
        return Path.home() / ".myclaude" / "memory" / ENTRYPOINT_NAME
    ...
    def user_mem_dir(self) -> Path:
        """用户级记忆目录（~/.myclaude/memory/）。"""
        return Path(self._user_mem_dir.rstrip(os.sep)) if self._user_mem_dir else Path.home() / ".myclaude" / "memory"

    @property
    def project_mem_dir(self) -> Path:
        """项目级记忆目录（<project>/.myclaude/memory/）。"""
    ...
    async def extract(
        ...
        if start > len(conversation.history):
            # Context compaction replaced the transcript with a shorter
            # summary+tail sequence; resume extraction from that new boundary.
            start = 0
        ...
        try:
            ...
            with scope("memory-extraction") if scope is not None else nullcontext():
        ...
        blocks = [
            block
            for block in re.split(r"(?m)^\s*---\s*$", collected)
            if "MEMORY_NAME:" in block
        ]
        for block in blocks:
            ...
            if not name or not body or not _SAFE_MEMORY_NAME_RE.fullmatch(name):
                continue
...
def _contains_sensitive_data(content: str) -> bool:
    return any(pattern.search(content) for pattern in _SENSITIVE_MEMORY_PATTERNS)
