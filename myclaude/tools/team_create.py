...
class TeamCreateTool(Tool):
    ...
    description = (
        ...
        "- A task has genuinely independent work that benefits from coordination "
        "and the lead has a concrete synthesis plan\n\n"
        "Do not create a team for a straightforward local change or ordinary review.\n\n"
        "## Team Workflow\n\n"
    ...
    async def execute(self, params: BaseModel) -> ToolResult:
        ...
        from myclaude.teams.backend_detect import BackendDetectionError
        ...
        coordinator_note = ""
        from myclaude.teams.coordinator import is_coordinator_mode
        if is_coordinator_mode(self._enable_coordinator_mode):
            from myclaude.agents.tool_filter import apply_coordinator_filter
