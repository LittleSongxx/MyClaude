...
class AgentTool(Tool):
    ...
    async def execute(self, params: BaseModel) -> ToolResult:
        ...
        from myclaude.agents.fork import ForkError, build_forked_messages
        from myclaude.agents.parser import AgentDef
        from myclaude.agents.tool_filter import clone_registry_for_fork, resolve_agent_tools
        from myclaude.agent import Agent as AgentClass
        ...
        pm_enum = restrict_child_mode(self._parent_agent.permission_mode, pm_enum)
        parent_checker = self._parent_agent.permission_checker
        checker = PermissionChecker(
            detector=(
                parent_checker.detector
                if parent_checker is not None
                else DangerousCommandDetector()
            ),
            sandbox=PathSandbox(self._parent_agent.work_dir),
            rule_engine=(
                parent_checker.rule_engine
                if parent_checker is not None
                else RuleEngine()
            ),
            mode=pm_enum,
        ...
        if p.subagent_type is None:
            from myclaude.context import clone_replacement_state
    ...
    async def _execute_as_teammate(self, p: AgentToolParams) -> ToolResult:
        ...
        from myclaude.agents.fork import ForkError, build_forked_messages
        from myclaude.agents.parser import AgentDef
        from myclaude.agents.tool_filter import build_teammate_tools
        from myclaude.agent import Agent as AgentClass
        from myclaude.conversation import ConversationManager
        from myclaude.permissions import (
            DangerousCommandDetector,
        ...
        from myclaude.teams.models import BackendType, TeammateInfo
        ...
        teammate_mode = restrict_child_mode(
            self._parent_agent.permission_mode, PermissionMode.BYPASS
        )
        checker = PermissionChecker(
            detector=(
                parent_checker.detector
    ...
    def _spawn_pane_teammate(
        ...
        from myclaude.teams.models import BackendType
        ...
        try:
            if backend == BackendType.TMUX:
                from myclaude.teams.spawn_tmux import spawn_tmux_teammate
                ...
                from myclaude.teams.spawn_iterm2 import spawn_iterm2_teammate
    ...
    async def _execute_with_worktree(self, p: AgentToolParams) -> ToolResult:
        ...
        from myclaude.worktree.integration import (
    ...
    def _create_client_for_model(self, model_alias: str) -> LLMClient | None:
        ...
        try:
            return create_client(
                config,
                usage_ledger=self._parent_agent.client.usage_ledger,
            )
        except Exception:
            return None
