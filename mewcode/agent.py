...
class Agent:
                ...
                if self._consolidator is not None:
                    self._spawn_background(
                        self._consolidator.maybe_run(
                            self.client, conversation.snapshot(), self.protocol
                        )
                    )
    ...
    async def run_to_completion(
        ...
        while True:
            ...
            # ToolSearch can enable deferred schemas during a previous iteration.
            # Rebuild the schema list every turn instead of freezing it at startup.
            tools = self.registry.get_all_schemas(self.protocol)
            request_attempt = 0
            while True:
                collector = StreamCollector()
                try:
                    ...
                    request_attempt += 1
                    if request_attempt >= MAX_REQUEST_RETRIES:
                        raise
                    wait = (
                        exc.retry_after
                        if isinstance(exc, RateLimitError) and exc.retry_after
            ...
            if response.stop_reason == "max_tokens":
                ...
                if not max_tokens_escalated:
                    ...
                    raise RuntimeError("Model repeatedly exhausted its output token limit")
                else:
                    output_recoveries += 1
                conversation.add_user_message(
                    "Output token limit hit. Resume directly from where you stopped. "
                    "Do not repeat previous content; break remaining work into smaller pieces."
                )
                continue
            output_recoveries = 0

            if response.stop_reason not in ("", "end_turn", "stop", "tool_use"):
                raise RuntimeError(
                    f"Model stopped unexpectedly: {response.stop_reason}"
                )
            ...
            for batch in partition_tool_calls(response.tool_calls, self.registry):
                ...
                if batch.concurrent:
                    executions = await self._execute_batch_parallel(batch.calls)
                else:
                    executions = [
                        await self._execute_single_tool_direct(batch.calls[0])
                    ]
                for execution in executions:
                    content = self._maybe_persist_or_truncate(
                        execution.tool_id, execution.result.output
                    )
                    tool_results.append(
                        ToolResultBlock(
